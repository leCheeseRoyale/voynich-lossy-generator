...
\author{%
  Showerhead\thanks{Pseudonymous author. Contact: @Formershowerhead on YouTube. Legal identity available to the editor upon request. AI-assisted methodology is disclosed in Section~\ref{sec:methods}.}
}
...
Our primary corpus is the Takahashi EVA transcription (version IT2a-n) in IVTFF 2.0 format, comprising 37,886 word tokens, 8,148 word types, and 5,645 hapax legomena (69.3\% of vocabulary). Pages are organized by manuscript section following conventional designations \citep[see][for a comprehensive overview]{Zandbergen2021}: Herbal ($n = 129$ pages), Stars ($n = 25$), Biological ($n = 19$), Pharmaceutical ($n = 16$), Zodiac ($n = 12$), Cosmological ($n = 10$), Astronomical ($n = 8$), and Text-only ($n = 6$), totaling 225 pages. Of the 129 herbal pages, 120 were available for botanical annotation (9 multi-part folios lacked IIIF image mappings).
...
\noindent where each slot $S_i$ takes a value from a finite inventory including an empty option (Figure~\ref{fig:slotmodel}; Table~\ref{tab:slotinventory}).
...
\hline
\textbf{$S_1$} & \textbf{$S_2$} & \textbf{$S_3$} & \textbf{$S_{4a}$} & \textbf{$S_{4b}$} & \textbf{$S_{4c}$} & \textbf{$S_5$} \\
\hline
Initial & Prefix & Gallows & Bench & E-series & Terminal & Suffix \\
\hline
5+$\emptyset$ & 6+$\emptyset$ & 11+$\emptyset$ & 3+$\emptyset$ & 4+$\emptyset$ & 2+$\emptyset$ & 16+$\emptyset$ \\
\hline
\multicolumn{3}{|c|}{\textit{Row-determined}} & \multicolumn{3}{c|}{\textit{Parametric}} & \textit{Row-det.} \\
\hline
\end{tabular}
\caption{The seven-slot generation model. Slots $S_1$, $S_2$, $S_3$, and $S_5$ are jointly determined by row selection from the lookup table. Slots $S_{4a}$, $S_{4b}$, and $S_{4c}$ provide within-row parametric variation. Numbers indicate fragment options per slot (plus empty).}
\label{fig:slotmodel}
\end{figure}
...
If the generation device encoded real content, pages depicting morphologically distinctive plants should show lower default rates---more slots filled with content fragments. Prior work by \citet{Sterneck2021} used topic modeling to establish that Voynich text clusters correlate with illustration types, suggesting a genuine text--image relationship. We tested our prediction on all 120 herbal pages annotated with 15 binary botanical features.
...
All analysis scripts, transcription data, botanical annotations, and reproduction instructions are available at \url{https://github.com/leCheeseRoyale/voynich-lossy-generator}.
...
\bibitem[Hodgins, 2011]{Hodgins2011}
Hodgins, G. (2011).
\newblock Measuring the radiocarbon content of the Voynich Manuscript.
\newblock Presented at the conference ``The Voynich Manuscript: Current Research and Future Directions,'' Villa Mondragone, Italy.
...
Zandbergen, R. (2021).
\newblock The Cardan grille approach to the Voynich MS taken to the next level.
